...
public class ContextAwarePoolExecutor extends ThreadPoolTaskExecutor {
    ...
    public <T> Future<T> submit(Callable<T> task) {
        ...
        return super.submit(new ContextAwareCallable<>(RequestContextHolder.currentRequestAttributes(), task));
    }

    @Override
    public Future<?> submit(Runnable task) {
        log.info("Future<?> - submit - Runnable");
        return super.submit(new ContextAwareRunnable(RequestContextHolder.currentRequestAttributes(), task));
    }
    ...
    public CompletableFuture<Void> submitCompletable(Runnable task) {
        ...
        return super.submitCompletable(new ContextAwareRunnable(RequestContextHolder.currentRequestAttributes(), task));
    }
